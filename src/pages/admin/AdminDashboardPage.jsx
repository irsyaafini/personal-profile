...
function StatCard({ icon: Icon, label, value, hint, to }) {
  return (
    <Link to={to} className="group block relative">
      <GlassSurface
        width="100%"
        height="100%"
        borderRadius={16}
        ...
        style={{ position: 'absolute', inset: 0, zIndex: 0, width: '100%', height: '100%', borderRadius: 16 }}
      />
  ...
  return (
    <>
      ...
      <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 gap-4">
        <StatCard icon={User}        label="Profile"     value={profile.data ? '✓' : '0'} hint={profile.data ? profile.data.full_name : 'Not set up yet'} to="/admin/profile" />
        <StatCard icon={Briefcase}   label="Experiences" value={exps.data?.length}    to="/admin/experiences" />
        <StatCard icon={Code2}       label="Skills"      value={skills.data?.length}  to="/admin/skills" />
        <StatCard icon={Camera}      label="Gallery"     value={gallery.data?.length} to="/admin/gallery" />
        <StatCard icon={FlaskConical} label="Research"   value={research.data?.length} to="/admin/research" />
        <StatCard
          icon={Inbox}
          label="Messages"
      ...
      <div className="mt-8">
        ...
        <div className="relative rounded-2xl overflow-hidden">
          ...
          <div className="relative z-10 divide-y divide-white/[0.06]">
            <Link to="/admin/profile"  className="flex items-center justify-between px-5 py-4 hover:bg-white/[0.03] transition">
              <span className="text-sm text-white/85">Edit your hero / bio</span>
              <ArrowUpRight className="h-4 w-4 text-white/30" />
            </Link>
            <Link to="/admin/gallery"  className="flex items-center justify-between px-5 py-4 hover:bg-white/[0.03] transition">
              <span className="text-sm text-white/85">Add a new gallery photo</span>
              <ArrowUpRight className="h-4 w-4 text-white/30" />
            </Link>
            <Link to="/admin/research" className="flex items-center justify-between px-5 py-4 hover:bg-white/[0.03] transition">
              <span className="text-sm text-white/85">Publish a research project</span>
              <ArrowUpRight className="h-4 w-4 text-white/30" />
            </Link>
            <Link to="/admin/messages" className="flex items-center justify-between px-5 py-4 hover:bg-white/[0.03] transition">
